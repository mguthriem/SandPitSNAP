...
from mantid.simpleapi import (AlignAndFocusPowder, AlignAndFocusPowderFromFiles, CloneWorkspace, ConvertUnits,
                              ...
                              Minus, NormaliseByCurrent, PreprocessDetectorsToMD, Rebin, RebinToWorkspace,ReplaceSpecialValues,
                              SaveAscii, SaveFocusedXYE, SaveGSS, SaveNexusProcessed, mtd,config)
...
class SNAPReduceNightly(DataProcessorAlgorithm):
    IPTS_dir = None
...
#MG modify init to first read a prescribed configuration file and use this to set some useful

        #can read info from mantidconfig dictionary. This can be set within the mantidinstance
        #by defining it like: config['something'] = True
        #it can then be used here.
        #so, thinking is to make it possible to set reduction configuration file location this way.
        
        if len(config['SNAPDRC'])==0:
        #   print('no config in mantid path using default config')
           SNAPReductionConfig = '/SNS/SNAP/shared/Calibration/1010/config1010_12-Oct-2021-091618.json'
        else:
        #   print('taking config from mantid path')
            SNAPReductionConfig = config['SNAPDRC']
        
        #load configuration file
        SNAPconfigDict = mg.getConfigDict(SNAPReductionConfig)
        # and extract useful info from it.
        dminsStr = SNAPconfigDict['dmins'].split(',')
        dmaxsStr = SNAPconfigDict['dmaxs'].split(',')
        dmins = [float(x) for x in dminsStr]
        dmaxs = [float(x) for x in dmaxsStr]
        dBinSize = float(SNAPconfigDict['dBinSize'])
        QBinSize = float(SNAPconfigDict['QBinSize'])
        dStart = min(dmins)
        dEnd = max(dmaxs)
        QStart = 2*np.pi/dEnd
        QEnd = 2*np.pi/dStart
        RebinParams = [dStart, dBinSize ,dEnd]
        RebinParamsQ = [QStart,QBinSize, QEnd]
        detCalName = SNAPconfigDict['detCalName']
        #InQ = SNAPconfigDict['InQ']
        ...
        self.declareProperty(name="Calibration", defaultValue="DetCal File",
        ...
        #self.declareProperty("DetCalFilename",detCalName,direction=Direction.Input,doc="detcal file from config")

        self.declareProperty(FloatArrayProperty("Binning", RebinParams),
                             "Min, Step, and Max of d-space bins.  Logarithmic binning is used if Step is negative.")

        nor_corr = ["None","From Workspace","On the fly (beta)"]
    ...
    def PyExec(self):
        ...
        for i, runnumber in enumerate(in_Runs):
            ...
            #MG: added rebin of redWS as align and focus doesn't match rebin
            Rebin(InputWorkspace=redWS, Params=self.alignAndFocusArgs['Params'],
            ...
            if normalizationWS is not None:
                ...
                RebinToWorkspace(WorkspaceToRebin=normalizationWS, WorkspaceToMatch=redWS, OutputWorkspace=normalizationWS)
...
AlgorithmFactory.subscribe(SNAPReduceNightly)