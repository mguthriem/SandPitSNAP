...
IPTS = 22004#27111 #ipts for sample data
VIPTS = 22079 #ipts for vanadium and its background (not always same as sample)
...
instFileLoc = '/SNS/snfs1/instruments/SNAP/IPTS-20627/shared/malcolm/data/SNAP_Definition.xml'

#locations on malcolm's macbook
#sharedDir = '/Users/malcolmguthrie/Documents/SNAP/IPTS12814/data2/' #equivalent to "shared" directory in SNS file structure 
#dataDir = '/Users/malcolmguthrie/Documents/SNAP/IPTS12814/data2/' #location of data, typically '../data' at SNS
#VDir = dataDir
#instFileLoc = '/Users/malcolmguthrie/Documents/SNAP/IPTS12814/data2/SNAP_Definition.xml'

#define some default values. These depend on detector angles and bandwidth settings.
#Configuration: 1 
#Description: Values currently used for "staggered" DAC detector set up configuration West low angle, East high angle 

dbinning ='0.5,-0.001,6' #dmin, dbin, dmax applied to all pixels data
MonTBinning = '3000,30,16500' #tof_min, tof_bin, tof_max for monitor spectra
nCol = 6
...
#Enter runs as comma separated list:
runs = [46756] #48746 900um 49854 (600um)

#measurement specific info:
msknm = 'snap46753msk2' #(DONT use .xml extension) will be ignored in mode 1
            #leave empty if no mask is required
Vrun = 46749
VBrun = 46750
vHeight = 0.06 #illuminated height of vanadium pin (cm)
vRad = 0.035 #radius of vanadium pin (cm)
detCalName = '/SNS/SNAP/shared/Calibration/1010/SNAP46742.detcal'  #full path
uStreamDiam = 1 #should be equal to either 1 or 2 depending on which diamond is upstream
#    Vrun = 46749
#    VBrun = 46750    
#detCalName = 'Lab6_46743_Calibed_Dec19.DetCal'
#set mode:
...
if mode==2 and useVCorrFile!= 0:# set up vanadium correction
    ...
    SmoothData(InputWorkspace='VCorr', OutputWorkspace='VCorr', NPoints='100')
    SetSampleMaterial(InputWorkspace='VCorr', ChemicalFormula='V')
    ConvertUnits(InputWorkspace='VCorr', OutputWorkspace='VCorr', Target='Wavelength')
    CylinderAbsorption(InputWorkspace='VCorr', OutputWorkspace='VCorr_a', AttenuationXSection=5.08, ScatteringXSection=5.10, CylinderSampleHeight=vHeight, CylinderSampleRadius=vRad, CylinderAxis='0,0,1')
    Divide(LHSWorkspace='VCorr', RHSWorkspace='VCorr_a', OutputWorkspace='VCorr')
    ConvertUnits(InputWorkspace='VCorr', OutputWorkspace='VCorr', Target='dSpacing')
...
#        outTrm= CreateWorkspace(DataX = xIn, DataY = yOut, NSpec = nHst, Distribution = False, UnitX = "d-Spacing ")
        ...
        nrmIndx = np.where(np.logical_and(dsMonX>=13000.0, dsMonX<=16000.0)) 
